...
return new CamanInstance(arguments,CamanInstance.Type.Unknown);case 2:if(typeof arguments[1]==='function'){if(Store.has(arguments[0])){return Store.execute(arguments[0],arguments[1]);}
return new CamanInstance(arguments,CamanInstance.Type.Unknown);}else{return new CamanInstance(arguments,CamanInstance.Type.Canvas);}
break;case 3:return new CamanInstance(arguments,CamanInstance.Type.Canvas);}};Caman.version={release:"3.2.2",date:"10/11/12"};Caman.DEBUG=false;Caman.autoload=!(typeof exports!=="undefined"&&exports!==null);Caman.toString=function(){return"Version "+Caman.version.release+", Released "+Caman.version.date;};Caman.remoteProxy="";Caman.Util=Util;CamanInstance=(function(){CamanInstance.Type={Image:1,Canvas:2,Unknown:3,Node:4};CamanInstance.toString=Caman.toString;function CamanInstance(args,type){var listener,_this=this;if(type==null){type=CamanInstance.Type.Canvas;}
...
CamanInstance.prototype.domLoaded=function(args,type){switch(type){case CamanInstance.Type.Image:return this.loadImage.apply(this,args);case CamanInstance.Type.Canvas:return this.loadCanvas.apply(this,args);case CamanInstance.Type.Unknown:return this.loadUnknown(args);}};CamanInstance.prototype.loadUnknown=function(args){var e;e=$(args[0]);switch(e.nodeName.toLowerCase()){case"img":return this.loadImage.apply(this,args);case"canvas":return this.loadCanvas(null,args[0],args[1]);}};CamanInstance.prototype.loadImage=function(sel,callback){var image,proxyURL,_ref,_this=this;if(callback==null){callback=function(){};}
if(typeof sel==="object"&&((_ref=sel.nodeName)!=null?_ref.toLowerCase():void 0)==="img"){image=sel;if(!image.id){image.id="caman-"+(Util.uniqid.get());}}else{image=$(sel);if(!image){throw"Could not find element "+sel;}
if(image.nodeName.toLowerCase()!=="img"){throw"Given element ID isn't an image: "+sel;}}
proxyURL=IO.remoteCheck(image.src);if(proxyURL){image.onload=function(){return _this.imageLoaded(image,callback);};return image.src=proxyURL;}else{if(image.complete){return this.imageLoaded(image,callback);}else{return image.onload=function(){return _this.imageLoaded(image,callback);};}}};CamanInstance.prototype.imageLoaded=function(image,callback){var attr,_i,_len,_ref;this.image=image;this.canvas=document.createElement('canvas');this.canvas.id=image.id;_ref=['data-camanwidth','data-camanheight'];for(_i=0,_len=_ref.length;_i<_len;_i++){attr=_ref[_i];if(this.image.getAttribute(attr)){this.canvas.setAttribute(attr,this.image.getAttribute(attr));}}
if(image.parentNode!=null){image.parentNode.replaceChild(this.canvas,this.image);}
return this.finishInit(callback);};CamanInstance.prototype.loadCanvas=function(url,sel,callback){var element,_ref;if(callback==null){callback=function(){};}
if(typeof sel==="object"&&((_ref=sel.nodeName)!=null?_ref.toLowerCase():void 0)==="canvas"){element=sel;if(!element.id){element.id="caman-"+(Util.uniqid.get());}}else{element=$(sel);if(!element){throw"Could not find element "+sel;}
if(element.nodeName.toLowerCase()!=="canvas"){throw"Given element ID isn't a canvas: "+sel;}}
return this.canvasLoaded(url,element,callback);};CamanInstance.prototype.canvasLoaded=function(url,canvas,callback){var proxyURL,_this=this;this.canvas=canvas;this.canvas.id;if(url!=null){this.image=document.createElement('img');this.image.onload=function(){return _this.finishInit(callback);};proxyURL=IO.remoteCheck(url);return this.image.src=proxyURL?proxyURL:url;}else{return this.finishInit(callback);}};CamanInstance.prototype.loadNode=function(file,callback){var img,_this=this;img=new Image();if(typeof file==="string"){file=fs.realpathSync(file);}
img.onload=function(){var context;_this.canvas=new Canvas(img.width,img.height);_this.canvas.id=Util.uniqid.get();context=_this.canvas.getContext('2d');context.drawImage(img,0,0);return _this.finishInit(callback);};img.onerror=function(err){throw err;};return img.src=file;};CamanInstance.prototype.finishInit=function(callback){var newHeight,newWidth,oldHeight,oldWidth;this.context=this.canvas.getContext("2d");if(this.image!=null){oldWidth=this.image.width;oldHeight=this.image.height;newWidth=this.canvas.getAttribute('data-camanwidth');newHeight=this.canvas.getAttribute('data-camanheight');if(newWidth||newHeight){if(newWidth){this.image.width=parseInt(newWidth,10);if(newHeight){this.image.height=parseInt(newHeight,10);}else{this.image.height=this.image.width*oldHeight/oldWidth;}}else if(newHeight){this.image.height=parseInt(newHeight,10);this.image.width=this.image.height*oldWidth/oldHeight;}}
...
Store.items={};Store.getId=function(search){if(typeof search==="object"){return search.id;}else{return search.slice(1);}};Store.has=function(search){return this.items[this.getId(search)]!=null;};Store.get=function(search){return this.items[this.getId(search)];};Store.put=function(name,obj){return this.items[name]=obj;};Store.execute=function(search,callback){return callback.call(this.get(search),this.get(search));};Store.flush=function(name){if(name==null){name=false;}
